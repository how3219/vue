...
<script>
import { margin, chartWidth, chartHeight } from '../config';
...
export default {
    ...
    setup(props) {
        ...
        const initDraw = (data, key) => {
            ...
            svg.append('g').attr('class', 'x axis').attr('transform', `translate(0,${chartHeight})`).call(xAxis);
            svg.append('g').attr('class', 'y axis').call(yAxis);

            svg.append('path').datum(data).attr('fill', 'steelblue').attr('d', area).attr('class', 'area');
            // data 를 이용해 그리는 방법은 3가지
            ...
            circle = svg
                ...
                .on('mouseover', (event, d) => {
                    ...
                    tooltip.html(content).style('left', event.pageX - 83 + 'px');
                    tooltip.html(content).style('top', event.pageY - 130 + 'px');
                })
                .on('mouseout', () => {
                    tooltip.transition().duration(200).style('opacity', 0);
                });

            circle.attr('cx', (d) => xScale(d.time)).attr('cy', (d) => yScale(d[key]));
        };
        const draw = (data, key) => {
            data.forEach((d) => (d.time = new Date(d.time)));
            const mn = d3.min(data, (d) => d[key]);
