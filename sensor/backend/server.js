...
const sensorController = require('./controller/sensorController.js');
...
const main = async () => {
    ...
    const timeInterval = setInterval(async () => {
        idx = await sensorController.sendDataAndSaveDB(io, sensorList, idx);
        console.log(
            `Send to user Current Sensor And Save DB :: ${util.getDate()} idx = ${idx} ${JSON.stringify(
                sensorList[idx],
            )}`,
        );
        idx += 1;
        if (idx === len) {
            ...
            io.emit('closeSensorService', 'close');
